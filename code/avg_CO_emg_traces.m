...
for iMuscle = 1:length(muscle_names)
    for iTp = unique([trialwise_states.tp])
        ...
        for iTrial = find([trialwise_states.tp] == iTp)
            ...
            trial_beginning_index = find(trialwise_states(iTrial).kinematic_timestamps == trial_beginning) -  pre_go_amount;
...
for iMuscle = 1:length(muscle_names)
    ...
    for iTp = unique([trialwise_states.tp])
        t =             -pre_go_amount:1:(length(avg_muscle_analysis(iTp).([muscle_names{iMuscle} '_avg']))-pre_go_amount-1);
