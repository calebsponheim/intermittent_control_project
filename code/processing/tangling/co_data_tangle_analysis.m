...
if normalize == 1
    for iTrial = 1:size(data_aligned_and_smoothed,2)
        for iUnit = 1:size(data_aligned_and_smoothed(iTrial).spikecountresamp,1)
            data_aligned_and_smoothed(iTrial).spikecountresamp(iUnit,:) = normalize(data_aligned_and_smoothed(iTrial).spikecountresamp(iUnit,:),'range');
        end
    end
end
...
if strcmp(meta.task,'center_out')
    times = data_aligned(1).ms_relative_to_trial_start;
    ...
    for iTrial = 1:size(data,2)
        ...
        data_to_average(data(iTrial).tp).speed(:,:,trial_count(data(iTrial).tp)) = data_aligned(iTrial).speed;
    end %iTrial
...
timepoints = 0:length(D_m1_CO(1).speed):length(out.X);
...
for iTP = unique([data.tp])
    ...
    ylabel('Tangling Amount')
        xticklabels(crop_window(1):50:crop_window(2))
    xlabel('Time(ms)')
