...
do
	...
	printf "#SBATCH --output=/project/nicho/caleb/git/intermittent_control_project/code/python_switching_models/out_files/%s_%s/LDS_%s.out\n" $subject $taskname  "%a" >> sbatch_dims_2-80_fold_${iFold}.sh
	printf "#SBATCH --output=/project/nicho/caleb/git/intermittent_control_project/code/python_switching_models/error_files/%s_%s/LDS_%s.err\n" $subject $taskname  "%a" >> sbatch_dims_2-80_fold_${iFold}.sh
