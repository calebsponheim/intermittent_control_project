...
do
	...
	do
		...
		printf "#SBATCH --output=/dali/nicho/caleb/git/intermittent_control_project/code/python_switching_models/out_files/%s_%s/rSLDS_%s_%i_%i.out\n" $subject $taskname  "%a" $iState $iFold>> sbatch_dims_2-80_state_${iState}_fold_${iFold}.sh
		printf "#SBATCH --output=/dali/nicho/caleb/git/intermittent_control_project/code/python_switching_models/error_files/%s_%s/rSLDS_%s_%i_%i.err\n" $subject $taskname  "%a" $iState $iFold>> sbatch_dims_2-80_state_${iState}_fold_${iFold}.sh
