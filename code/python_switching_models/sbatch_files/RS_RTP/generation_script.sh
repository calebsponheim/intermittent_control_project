...
do
	...
	do
		...
		printf "#SBATCH --time=36:00:00\n#SBATCH --mem-per-cpu=48G\n#SBATCH--account=pi-nicho\n#SBATCH --partition=caslake" >> sbatch_dims_2-80_state_${iState}_fold_${iFold}.sh
		printf "module load python/anaconda-2021.05\nsource activate /project/nicho/caleb/git/intermittent_control_project/data/ssm_midway_python_environment/\n" >> sbatch_dims_2-80_state_${iState}_fold_${iFold}.sh
		printf "python /project/nicho/caleb/git/intermittent_control_project/code/python_switching_models/run_param_search.py \$SLURM_ARRAY_TASK_ID %i %i %s %s" $iFold $iState $subject $taskname >> sbatch_dims_2-80_state_${iState}_fold_${iFold}.sh
	done
done