...
for iTrial = 1:length(trInd_test)
    ...
    for iSegment = 1:size(speed_plot(iTrial).neural_state_segment,2)
        if ~isempty(speed_plot(iTrial).neural_state_segment{iSegment})
            ...
            segment_beginning = min(speed_plot(iTrial).neural_state_segment{iSegment})+.010 - .025;
            segment_end = max(speed_plot(iTrial).neural_state_segment{iSegment})+.010 + .025;
...
for iTrial = datasample(1:length(trInd_test),15)
    for iSegment = 1:size(speed_plot(iTrial).segment_state_number,2)
        if ~isempty(speed_plot(iTrial).segment_kinematic_timestamps{iSegment}) && speed_plot(iTrial).segment_state_number(iSegment) ~= 0
            ...
            % Segment Direction
            beginning_of_segment(1) = speed_plot(iTrial).segment_kinematic_x{iSegment}(1);
            beginning_of_segment(2) = speed_plot(iTrial).segment_kinematic_y{iSegment}(1);
            end_of_segment(1) = speed_plot(iTrial).segment_kinematic_x{iSegment}(end);
            end_of_segment(2) = speed_plot(iTrial).segment_kinematic_y{iSegment}(end);
