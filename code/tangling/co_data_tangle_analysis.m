...
if strcmp(meta.task,'center_out')
    times = data(1).ms_relative_to_trial_start;
    analyzeTimes = times; %we wanna analyze the whole dang thing, bobby;
    
    for iTP = unique([data.tp])
        data_to_average(iTP).data = zeros(size(data(1).spikecountresamp,1),size(data(1).spikecountresamp,2),sum([data.tp] == iTP));
    end
    trial_count = zeros(1,numel(unique([data.tp])));
    
    for iTrial = 1:size(data,2)
        trial_count(data(iTrial).tp) = trial_count(data(iTrial).tp) + 1;
        data_to_average(data(iTrial).tp).data(:,:,trial_count(data(iTrial).tp)) = data(iTrial).spikecountresamp;
    end %iTrial
...
[ Q, out] = tangleAnalysis( D_m1_CO, meta.bin_size);
